...
class LogRepository:
    ...
    def get_by_user(self, user_key: str, filters: LogFilter) -> List[dict]:
        ...
        if filters.user_key is not None:
            filters_list.append(
                "("
                "n.user_key == @filter_user_key "
                "OR n.granted_by_key == @filter_user_key "
                "OR n.granted_to_key == @filter_user_key"
                ")"
            )
            bind_vars["filter_user_key"] = filters.user_key
        ...
        if filters.search is not None:
            filters_list.append("""
                (
                    CONTAINS(LOWER(n.note_key), LOWER(@search)) OR
                    CONTAINS(LOWER(n.user_key), LOWER(@search)) OR
